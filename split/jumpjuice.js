...
let profN=0,profSim=0,profDraw=0,profSkip=0;
...
/* DOM -> OPT -> SAVE. Persisting here means every settings change survives a
   ...
   save file. */
let optsReady=false;
/* True once a save has been read that contained a real, valid `scheme`.
   Only then is the player's control choice considered deliberate. */
let schemeWasSaved=false;
function syncOpts(){
...
function applyOpts(){
  ...
  /* On-screen buttons are the default for a NEW player on a touch device:
     they are the layout the game is designed around, and discovering the
     gesture scheme by accident is worse than being shown the controls.
     Gated on schemeWasSaved so a returning player who deliberately chose
     gestures is never overridden — the flag is only set when a save
     actually contained a valid scheme. Desktop is unaffected: TOUCH is
     false there, and keyboard control is independent of this setting. */
  if(!schemeWasSaved&&TOUCH)OPT.scheme="buttons";
  optsReady=true;
  syncOpts();
  setVol(OPT.vol);
  syncSchemeUI();
  ...
  if(ro&&typeof ro==="object"){
    for(const k in OPT_DEF){
      if(k==="vol"){
        ...
        /* Remember whether the player ever actually HAD a scheme saved.
           Without this flag "never chose" and "chose gestures" are
           indistinguishable after migration, and the touch default below
           would silently overwrite a deliberate choice on every load. */
        if(SCHEMES.includes(ro.scheme)){SAVE.opt.scheme=ro.scheme;schemeWasSaved=true;}
        else SAVE.opt.scheme=OPT_DEF.scheme;
...
function sfx(k,x){
  ...
  /* Three layers, ~150ms total, deliberately unlike jump (rising blip),
     land (low thud) or hurt (long sawtooth fall):
       1. bright filtered-noise air attack — the "sh" of the whoosh
       2. fast downward pitch sweep — the sense of leaving somewhere
       3. very short low body — weight, so it is not just hiss
     Levels are kept well under 1.0 combined so the mix cannot clip. */
  case"dash":     nzs(.11,.13,2600,1.6);
                  bl(1150,300,.13,"sawtooth",.10);
                  bl(260,120,.07,"sine",.11);break;
...
const BREWS={
  ...
  rainbow:{n:"Rainbow Juice",icon:"🌈",col:"#FF8AD8",
    ...
    t:"Juice Mode lasts 60% longer and orbs are worth triple"}};
...
const ACH=[
 ...
 /* Text is derived from the roster: it read "all 30 runners" while the check
    used CHARS.length, which is 34 — the award asked for four more heroes
    than it named. A GETTER, not a plain string: CHARS is declared after
    this table, so evaluating it eagerly here is a temporal-dead-zone crash
    at load. Every reader uses `a.t` as a property, so nothing else changes. */
 {id:"a9",get t(){return "Unlock all "+CHARS.length+" heroes";},
  chk:()=>SAVE.unlocked.length>=CHARS.length},
 ...
 {id:"a11",t:"Collect 2000 juice drops",chk:()=>SAVE.totalOrbs>=2000},
...
/*2*/{id:"Frog",hex:"#7FD46A",eye:"#FFFFFF",rar:0,pas:"walljump",draw(c,w,h,f,k,t){
...
/*4*/{id:"Crab",hex:"#E2603F",eye:"#FFFFFF",rar:0,pas:"combo",draw(c,w,h,f,k,t){
...
/*5*/{id:"Shroom",hex:"#D8BE8A",eye:"#0B1226",rar:0,pas:"fruity",draw(c,w,h,f,k,t){
...
/*10*/{id:"Cactus",hex:"#4FA85F",eye:"#0B1226",rar:1,pas:"aircontrol",draw(c,w,h,f,k,t){
...
/*17*/{id:"Squid",hex:"#C77BE0",eye:"#FFFFFF",rar:2,pas:"shield",draw(c,w,h,f,k,t){
...
/*30*/{id:"Bolt",hex:"#FFE24A",eye:"#0B1226",rar:2,pas:"speed",draw(c,w,h,f,k,t){
  ...
  c.fillStyle=k.b;c.beginPath();
  c.moveTo(w*.5,0);c.lineTo(w,h*.44);c.lineTo(w*.62,h*.44);c.lineTo(w*.84,h);
  c.lineTo(w*.16,h*.5);c.lineTo(w*.5,h*.5);c.closePath();c.fill();
  c.fillStyle="rgba(255,255,255,"+(.4+z)+")";
  c.beginPath();c.moveTo(w*.5,h*.08);c.lineTo(w*.84,h*.42);c.lineTo(w*.56,h*.42);c.closePath();c.fill();
  ey(c,w*.52+f*w*.06,h*.3,w*.055,k.d);}},
/*31*/{id:"Frost",hex:"#A8ECFF",eye:"#1B4A66",rar:3,pas:"slow",draw(c,w,h,f,k,t){
  c.fillStyle=k.b;c.beginPath();c.roundRect(w*.08,h*.16,w*.84,h*.84,w*.26);c.fill();
  c.strokeStyle="rgba(255,255,255,.85)";c.lineWidth=1.6;
  for(let i=0;i<3;i++){const a=t/50+i*2.09,cx=w*.5,cy=h*.2;
    c.beginPath();c.moveTo(cx,cy);
    c.lineTo(cx+Math.cos(a)*w*.34,cy+Math.sin(a)*w*.34);c.stroke();}
  c.fillStyle="rgba(255,255,255,.6)";
  for(let i=0;i<4;i++){const sy=h*(.34+i*.16);
    c.beginPath();c.moveTo(w*.16,sy);c.lineTo(w*.26,sy+h*.09);c.lineTo(w*.16,sy+h*.06);c.closePath();c.fill();
    c.beginPath();c.moveTo(w*.84,sy);c.lineTo(w*.74,sy+h*.09);c.lineTo(w*.84,sy+h*.06);c.closePath();c.fill();}
  ey(c,w*.36+f*w*.05,h*.5,w*.075,k.e);ey(c,w*.64+f*w*.05,h*.5,w*.075,k.e);
  c.strokeStyle=k.e;c.lineWidth=1.3;c.beginPath();
  c.moveTo(w*.4,h*.7);c.quadraticCurveTo(w*.5,h*.64,w*.6,h*.7);c.stroke();}},
/*32*/{id:"Inferno",hex:"#FF5A2B",eye:"#FFE9A0",rar:4,pas:"firelord",draw(c,w,h,f,k,t){
  const fl=Math.sin(t/3.4)*.16, fl2=Math.sin(t/2.1+1)*.12;
  c.fillStyle="rgba(255,190,80,.55)";
  c.beginPath();c.moveTo(w*.5,-h*.34-fl*h);
  c.quadraticCurveTo(w*1.02,h*.16,w*.5,h*.44);
  c.quadraticCurveTo(-w*.02,h*.16,w*.5,-h*.34-fl*h);c.fill();
  c.fillStyle=k.b;c.beginPath();c.moveTo(w*.5,-h*.1-fl2*h);
  c.quadraticCurveTo(w*.94,h*.3,w*.78,h*.78);
  c.quadraticCurveTo(w*.5,h*1.02,w*.22,h*.78);
  c.quadraticCurveTo(w*.06,h*.3,w*.5,-h*.1-fl2*h);c.fill();
  c.fillStyle="rgba(255,120,40,.9)";
  c.beginPath();c.ellipse(w*.5,h*.66,w*.26,h*.2,0,0,7);c.fill();
  ey(c,w*.36+f*w*.05,h*.44,w*.075,k.e);ey(c,w*.64+f*w*.05,h*.44,w*.075,k.e);
  ey(c,w*.36+f*w*.05,h*.44,w*.032,"#0B1226");ey(c,w*.64+f*w*.05,h*.44,w*.032,"#0B1226");}},
/*33*/{id:"Nature",hex:"#7BD86A",eye:"#22402A",rar:3,pas:"harvest",draw(c,w,h,f,k,t){
  const sw=Math.sin(t/22)*.06;
  c.strokeStyle="#4E8F3E";c.lineWidth=2.4;
  c.beginPath();c.moveTo(w*.5,h*.2);c.quadraticCurveTo(w*(.5+sw),h*.06,w*.62,-h*.04);c.stroke();
  c.fillStyle="#5FBF4F";
  c.beginPath();c.ellipse(w*.68,-h*.06,w*.16,h*.07,-.6,0,7);c.fill();
  c.beginPath();c.ellipse(w*.34,h*.06,w*.15,h*.065,.7,0,7);c.fill();
  c.fillStyle=k.b;c.beginPath();c.ellipse(w*.5,h*.6,w*.46,h*.42,0,0,7);c.fill();
  c.fillStyle="rgba(255,255,255,.22)";
  for(let i=0;i<3;i++){c.beginPath();
    c.ellipse(w*(.3+i*.2),h*(.44+(i%2)*.2),w*.06,h*.045,.4,0,7);c.fill();}
  ey(c,w*.36+f*w*.05,h*.56,w*.08,"#FFFFFF");ey(c,w*.64+f*w*.05,h*.56,w*.08,"#FFFFFF");
  ey(c,w*.36+f*w*.05,h*.56,w*.036,k.e);ey(c,w*.64+f*w*.05,h*.56,w*.036,k.e);
  c.strokeStyle=k.e;c.lineWidth=1.2;c.beginPath();
  c.arc(w*.5,h*.68,w*.11,.25,Math.PI-.25);c.stroke();}}
];
...
const PASS={
 ...
 slow:{t:"enemies move 45% slower · immune to ice"},
 ...
 icegrip:{t:"immune to ice · sure-footed"},
...
/* Vertical shake is tracked separately so a dash can be weighted toward the
   horizontal (it is a horizontal move — shaking the screen up and down for
   it reads as an impact rather than speed). Existing callers set `shake`
   only and get the original uniform behaviour, because shakeY falls back
   to shake when it has not been set higher. shakeT gives an impulse a
   defined lifetime so a held dash cannot keep re-arming it. */
let shakeY=0,shakeT=0;
...
/* ══════ GAME FEEL ════════════════════════════════════════════════
   Every tunable that shapes how jumping and dashing FEEL, in one place.
   Nothing here changes physics — the collision box, gravity, coyote time,
   jump buffering and variable jump height are untouched. These values only
   drive what the player sees, hears and feels.

   Why the split matters: an earlier pass set P.sx/P.sy from four unrelated
   places, so a landing squash could be erased mid-frame by the run cycle
   and a dash pose could be overwritten by a double jump. The visual state
   below is priority-ordered so that cannot happen. */
const FX={
  /* ── take-off ──
     Narrower and taller: the body loads upward. Anchored at the FEET in the
     renderer, so the character does not appear to teleport when it scales. */
  jumpSX:.80, jumpSY:1.26,
  jumpDust:7,                 /* particles per grounded take-off          */
  /* ── double jump ──
     Stronger stretch than a ground jump so the second one reads as more,
     plus a ring the ground jump never gets. */
  dblSX:.74, dblSY:1.34,
  dblBurst:10,                /* 7-12 per the brief                       */
  dblKick:.5,                 /* existing rotational kick, preserved       */
  dblShake:1.6,               /* tiny — a double jump is not an impact     */
  /* ── landing tiers ──
     Selected from impact speed as a fraction of terminal velocity. Three
     tiers so a drop from one platform does not feel like a 20m fall. */
  landSoft:.28, landHard:.62, /* imp<soft = soft, >hard = hard, else med   */
  landDust:[4,9,15],          /* soft / medium / hard particle counts      */
  landSquash:[.14,.26,.42],   /* how much the body widens                  */
  landShake:[0,2.2,5.5],      /* hard sits in the brief's 4-7 band         */
  landStop:[0,0,2],           /* hit-stop frames — hard landings only      */
  landRebound:.055,           /* overshoot past 1 before settling          */
  /* ── recovery ──
     Springy rather than linear: the body returns fast then eases, which is
     what makes squash read as elastic instead of mechanical. */
  squashSpring:.26, squashDamp:.62,
  /* ── dash ── */
  dashSX:1.30, dashSY:.80,    /* stretched along travel                    */
  dashLean:.34,               /* radians toward the dash direction         */
  dashShakeX:3.4, dashShakeY:1.5, /* horizontal-weighted, per the brief    */
  dashShakeMs:110,            /* ~7 frames at 60Hz (brief: 80-130ms)       */
  dashBurst:14,               /* particles at the moment of the dash       */
  dashStream:2,               /* per frame during the sustained dash       */
  dashTrailGap:7,             /* px travelled between afterimages — spacing
                                 by DISTANCE stops the trail becoming a row
                                 of identical stacked duplicates           */
  dashLead:58,                /* extra camera look-ahead while dashing     */
  dashLeadEase:.06,           /* how fast the camera adopts/returns to it  */
  dashRingMs:180,             /* energy ring lifetime (brief: 120-220ms)   */
  /* ── damage ── */
  hurtSX:1.34, hurtSY:.72,
};
/* Visual-only player state. Kept apart from P's physics fields on purpose:
   nothing here may ever be read by collision or movement code. */
const PV={
  action:"none",              /* what is currently driving the pose        */
  actionT:0,                  /* frames remaining at this priority         */
  sxv:0, syv:0,               /* spring velocities for the squash          */
  lean:0, leanT:0,            /* body tilt, used by dash                   */
  dashGlow:0,                 /* 0..1, fades the dash ring + edge highlight*/
  dashDir:1,
  landStrength:0,             /* 0..1, last landing impact                 */
  trailX:0, trailY:0,         /* last afterimage position (for spacing)    */
  camLead:0,                  /* current extra look-ahead                  */
};
/* Priority: a lower number may not overwrite a higher one while it is
   still playing. Damage always wins — being hit must read even if it
   happens mid-dash. */
const PV_PRI={none:0,run:1,jump:2,dbl:3,dash:4,land:5,hurt:6};
function setPose(action,frames,sx,sy){
  const cur=PV_PRI[PV.action]||0, next=PV_PRI[action]||0;
  if(PV.actionT>0&&next<cur)return false;   /* higher-priority pose holds  */
  PV.action=action;PV.actionT=frames;
  if(sx!==undefined){P.sx=sx;P.sy=sy;PV.sxv=0;PV.syv=0;}
  return true;
}
/* Explicit reset. Deliberately NOT setPose("none") — that would have to be
   allowed to override any priority, which would give game code a way to
   cancel a damage reaction by accident. Only the run reset and tests use
   this. */
function clearPose(){PV.action="none";PV.actionT=0;PV.sxv=0;PV.syv=0;}
...
/* Fields beyond the original x/y/vx/vy/life/r/gold are pre-allocated on
   every slot, not added on demand — a particle that grows a property at
   runtime deoptimises the whole pool's hidden class.
     k    kind: 0 square (original), 1 speed streak, 2 juice droplet
     ang  streak angle, len/wid streak dimensions
     col  explicit colour, or "" to use the gold/neutral default
     drag per-frame velocity retention                                    */
for(let i=0;i<PMAX;i++)PART[i]={x:0,y:0,vx:0,vy:0,life:0,r:1,gold:false,
                                k:0,ang:0,len:0,wid:1,col:"",drag:1,fade:1};
let pIdx=0;
function spark(x,y,vx,vy,r,gold,life){
  const q=PART[pIdx];pIdx=(pIdx+1)%PMAX;
  q.x=x;q.y=y;q.vx=vx;q.vy=vy;q.r=r;q.gold=!!gold;q.life=life||1;
  q.k=0;q.col="";q.drag=1;q.fade=1;q.len=0;q.ang=0;q.wid=1;return q;}
/* Directional particle used by the dash: a stretched streak or a droplet
   that travels opposite the movement and fades fast. */
function streak(x,y,vx,vy,len,wid,col,life,fade,kind){
  const q=PART[pIdx];pIdx=(pIdx+1)%PMAX;
  q.x=x;q.y=y;q.vx=vx;q.vy=vy;q.life=life||1;q.r=wid;
  q.k=kind===undefined?1:kind;q.ang=Math.atan2(vy,vx);q.len=len;q.wid=wid;
  q.col=col||"";q.gold=false;q.drag=.90;q.fade=fade||1.8;return q;}
...
/* ── trail: ring buffer ──
   Each entry stores enough to reproduce the pose it was captured in, so an
   afterimage matches what the character actually looked like rather than
   re-drawing the CURRENT pose at an old position. */
const TMAX=22,TRAIL=new Array(TMAX);
for(let i=0;i<TMAX;i++)TRAIL[i]={x:0,y:0,rot:0,life:0,sx:1,sy:1,face:1,dash:0,juice:false};
let tIdx=0;
...
function tryDash(){
  ...
  P.vy=0;dashedOnce=true;
  triggerDashFeedback();
}
/* ══════ DASH FEEDBACK ════════════════════════════════════════════
   Called EXACTLY ONCE when a dash begins. Everything that should happen
   at the moment of the dash lives here — sound, shake, burst, pose, ring
   — so none of it can end up in the per-frame path, which is what made
   the original re-trigger sparks every frame it was held.

   The sustained part of the dash (trail, streaming particles) is in
   dashStep(), called once per simulated frame while P.dash>0. */
function triggerDashFeedback(){
  const cx=P.x+P.w/2, cy=P.y+P.h/2, dir=P.face;
  PV.dashDir=dir;
  /* pose: stretched along travel, leaning into it */
  setPose("dash",Math.max(10,P.dash|0),FX.dashSX,FX.dashSY);
  PV.lean=FX.dashLean*dir;PV.leanT=Math.max(12,P.dash|0);
  PV.dashGlow=1;
  PV.trailX=P.x;PV.trailY=P.y;
  /* camera impulse — horizontal-weighted, decays smoothly, never in Calm
     Mode or under prefers-reduced-motion */
  if(!calm()){
    const k=saver()?.6:1;
    shake=Math.max(shake,FX.dashShakeX*k);
    shakeY=Math.max(shakeY,FX.dashShakeY*k);
    shakeT=Math.round(FX.dashShakeMs*.06);       /* ms -> frames at 60Hz */
  }
  sfx("dash");buzz(14);
  /* start burst: thrown BACKWARD, opposite the dash */
  const col=juice>0?juiceCol():me().hex;
  const n=saver()?Math.round(FX.dashBurst*.5):FX.dashBurst;
  for(let i=0;i<n;i++){
    const sp=3+Math.random()*5.5, spread=(Math.random()-.5)*2.6;
    streak(cx-dir*4,cy+(Math.random()-.5)*P.h*.7,
           -dir*sp,spread,
           7+Math.random()*13, 1.2+Math.random()*1.4,
           col, 1, 2.1, Math.random()<.35?2:1);
  }
  /* a couple of neutral dust motes so the burst is not one flat colour */
  if(!saver())for(let i=0;i<4;i++)
    spark(cx-dir*6,cy+(Math.random()-.5)*P.h,-dir*(1+Math.random()*2),
          (Math.random()-.5)*2.4,1.4+Math.random()*1.6,juice>0);
  /* compressed energy ring at the hero, tinted by juice or hero colour */
  ring(cx,cy,col,3.2);
}
/* ══════ JUMP / LAND FEEDBACK ═════════════════════════════════════
   Each is one call at the moment of the event, mirroring the dash. */

/* Ground colour under the player, so dust in snow, ash and cloud reads
   differently from dust on ordinary ground. Falls back to the world's
   platform colour when the player is not over anything. */
function groundTint(){
  const wl=worldAt(dist);
  if(P.onPl&&P.onPl.spike)return wl.p;
  return wl.p||"#A8B8D8";
}
/* Take-off puff at BOTH feet, thrown outward and slightly up. */
function takeoffDust(){
  const n=saver()?4:FX.jumpDust, col=groundTint(), fy=P.y+P.h;
  for(let i=0;i<n;i++){
    const side=i%2?1:-1;
    const q=spark(P.x+P.w/2+side*(3+Math.random()*4),fy,
                  side*(.6+Math.random()*1.7),-(.3+Math.random()*1.3),
                  1.4+Math.random()*2,false,.9);
    q.col=col;q.drag=.94;q.fade=1.5;
  }
}
/* The double jump gets a ring and a circular burst the ground jump never
   gets, so the two read as different actions rather than one repeated. */
function doubleJumpBurst(){
  setPose("dbl",11,FX.dblSX,FX.dblSY);
  const cx=P.x+P.w/2, cy=P.y+P.h, col=juice>0?juiceCol():me().hex;
  ring(cx,cy,col,2.6);
  const n=saver()?6:FX.dblBurst;
  for(let i=0;i<n;i++){
    const a=(i/n)*Math.PI*2, sp=1.5+Math.random()*1.9;
    const q=spark(cx,cy,Math.cos(a)*sp,Math.abs(Math.sin(a))*sp*.8,
                  1.5+Math.random()*1.5,juice>0,1);
    q.col=col;q.drag=.93;
  }
  /* a double jump is not an impact — a tiny nudge only, never in Calm Mode */
  if(!calm())shake=Math.max(shake,FX.dblShake);
}
/* Landing, in three tiers chosen from impact speed. Returns the tier so the
   caller can keep its existing perfect-landing and shockwave logic. */
function landingFeedback(imp,groundY){
  const tier=imp<FX.landSoft?0:(imp<FX.landHard?1:2);
  const col=groundTint(), cx=P.x+P.w/2;
  PV.landStrength=imp;
  /* squash scaled within the tier, so a landing still varies inside a band */
  const s=FX.landSquash[tier]*(.7+imp*.5);
  setPose("land",tier===2?14:10,1+s,1-s*.85);
  /* dust: wider and flatter the harder the landing */
  let n=FX.landDust[tier];
  if(saver())n=Math.max(3,Math.round(n*.55));
  for(let i=0;i<n;i++){
    const side=i%2?1:-1, spread=tier===2?3.6:tier===1?2.6:1.7;
    const q=spark(cx+side*(2+Math.random()*5),groundY,
                  side*(.5+Math.random()*spread),-(Math.random()*(1+tier*.8)),
                  1.4+Math.random()*(1.6+tier*.9),false,1);
    q.col=col;q.drag=.93;q.fade=1.4;
  }
  if(tier===2){
    /* a hard landing gets a ground ring and a brief flash AT THE FEET —
       never a full-screen flash, which would wash out the play area */
    ring(cx,groundY,col,4.2);
    for(let i=0;i<4;i++){
      const q=streak(cx+(i%2?1:-1)*(6+Math.random()*10),groundY,
                     (i%2?1:-1)*(2+Math.random()*2.4),0,
                     9+Math.random()*9,1.5,col,.9,2.2,1);
      q.drag=.86;
    }
  }
  if(!calm()){
    const k=saver()?.7:1;
    if(FX.landShake[tier]>0)shake=Math.max(shake,FX.landShake[tier]*imp*k);
    if(FX.landStop[tier]>0)hstop=Math.max(hstop,FX.landStop[tier]);
  }
  sfx("land",imp);
  if(tier===2)buzz(16); else if(tier===1)buzz(9);
  return tier;
}

/* Per-frame dash upkeep: afterimages spaced by DISTANCE TRAVELLED and a
   thin stream of speed particles. Spacing by distance rather than spawning
   every frame is what stops the trail reading as a row of solid duplicate
   characters when the player dashes slowly or stands still. */
function dashStep(){
  if(P.dash<=0)return;
  const cx=P.x+P.w/2, cy=P.y+P.h/2, dir=PV.dashDir;
  const col=juice>0?juiceCol():me().hex;
  /* afterimages: skipped entirely in Calm Mode (repeated ghosting is the
     motion-sensitivity problem) and thinned under Battery Saver */
  if(!calm()){
    const gap=FX.dashTrailGap*(saver()?1.9:1);
    const dx=P.x-PV.trailX, dy=P.y-PV.trailY;
    if(dx*dx+dy*dy>=gap*gap){
      const t=TRAIL[tIdx];tIdx=(tIdx+1)%TMAX;
      t.x=P.x;t.y=P.y;t.rot=P.rot;t.life=1;
      t.sx=P.sx;t.sy=P.sy;t.face=P.face;t.dash=1;t.juice=juice>0;
      PV.trailX=P.x;PV.trailY=P.y;
    }
  }
  /* sustained speed particles: fewer than the start burst, per the brief */
  const per=saver()?1:FX.dashStream;
  for(let i=0;i<per;i++){
    if(Math.random()>.75)continue;
    streak(cx-dir*(2+Math.random()*8),cy+(Math.random()-.5)*P.h*.85,
           -dir*(2.5+Math.random()*3.5),(Math.random()-.5)*1.3,
           6+Math.random()*10, 1+Math.random()*1.2,
           col, .85, 2.4, Math.random()<.3?2:1);
  }
}
...
/* A very short tick on the steering buttons only. Jump and dash already
   buzz from their own feedback (buzz(14) in the dash, landings in the jump
   arc), so ticking here too would double up on the same press. */
const padTick=()=>buzz(6);
bindHold("pLeft", ()=>{padL=true;padTick();}, ()=>{padL=false;});
bindHold("pRight",()=>{padR=true;padTick();}, ()=>{padR=false;});
...
function syncSchemeUI(){
  ...
  /* The pad exists only during ACTIVE gameplay — never on a menu, never
     while paused, never over the run-over screen. */
  $("pad").classList.toggle("show",on&&usedTouch&&ST==="play"&&!paused);
}
/* ══════ CONTROL FEEDBACK ═════════════════════════════════════════
   Drives the dash cooldown ring and the double-jump indicator from real
   game state. Called once per rendered frame, but every write is guarded
   by a cached previous value: touching classList or a custom property on
   an unchanged element still costs style invalidation, and this runs at
   60Hz. */
let _cdShown=-1,_dblShown=null,_coolShown=null;
function syncPadState(){
  const pad=$("pad");
  if(!pad||!pad.classList.contains("show"))return;
  const dashBtn=$("pDash"),ringEl=$("pDashRing"),jumpBtn=$("pJump");
  if(!dashBtn||!jumpBtn)return;
  /* dash cooldown 0..1 — quantised to 24 steps so the custom property is
     not rewritten every single frame */
  const maxCd=DASH_CD*(has("dash")?.6:1);
  const cd=maxCd>0?Math.min(1,1-P.dashCd/maxCd):1;
  const q=Math.round(cd*24);
  if(q!==_cdShown){
    _cdShown=q;
    if(ringEl)ringEl.style.setProperty("--cd",(q/24).toFixed(3));
  }
  const cooling=P.dashCd>0;
  if(cooling!==_coolShown){
    _coolShown=cooling;
    dashBtn.classList.toggle("cool",cooling);
    if(!cooling){
      /* one pulse on becoming ready, then the class is removed so it does
         not keep animating while the player has dash available */
      dashBtn.classList.remove("ready");
      void dashBtn.offsetWidth;                 /* restart the animation */
      dashBtn.classList.add("ready");
      setTimeout(()=>dashBtn.classList.remove("ready"),460);
    }
  }
  /* double jump available: airborne, under the cap, and not wall-locked */
  const maxJ=juice>0?99:(RMOD==="NODBL"?1:2);
  const dblReady=!P.grd&&P.jumpsUsed<maxJ&&ST==="play";
  if(dblReady!==_dblShown){
    _dblShown=dblReady;
    jumpBtn.classList.toggle("dbl",dblReady);
  }
}
...
function start(){
  ...
  /* Reset the visual state with the run, or a pose left over from the last
     death (or a dash that was mid-flight) bleeds into the new one. */
  clearPose();PV.lean=0;PV.leanT=0;
  PV.dashGlow=0;PV.landStrength=0;PV.camLead=0;PV.trailX=P.x;PV.trailY=P.y;
  shakeY=0;shakeT=0;
...
function hurt(reason){
  ...
  /* Highest priority — being hit must read even mid-dash, and nothing may
     overwrite it until it has played out. */
  setPose("hurt",14,FX.hurtSX,FX.hurtSY);
  PV.lean=0;PV.leanT=0;PV.dashGlow=0;
...
function die(reason){
  ...
  rows.push(["Coins collected","◈ "+(earn-turbo-(dist>=500?150:0))]);
  ...
  if(next){
    ...
    rows.push(["Next hero",next.id+"  "+pc+"%  (◈"+SAVE.coins+" / "+next.cost+")"]);
  }
...
function stepParticles(){
  for(let i=0;i<PMAX;i++){const q=PART[i];if(q.life<=0)continue;
    q.x+=q.vx;q.y+=q.vy;
    /* Speed streaks are near-weightless and drag hard — they should hang in
       the air behind the dash, not arc to the floor like debris. */
    q.vy+=q.k===1?.03:.16;
    q.vx*=q.k===1?q.drag:(q.drag*.96);
    if(q.k!==1)q.vy*=q.drag;
    q.life-=.035*q.fade;}
...
function step(){
  ...
  if(wants){
    if(grace&&P.vy>=0){
      ...
      /* Physics is already applied above — the pose is purely visual and
         adds no anticipation delay. */
      setPose("jump",9,FX.jumpSX,FX.jumpSY);sfx("jump");jumps++;
      takeoffDust();
      ...
      P.vy=JV*.88;bufAt=-999;jumpFrame=frame;P.jumpsUsed++;sfx("dbl");jumps++;
      P.rot+=P.face*FX.dblKick;doubleJumped=true;
      doubleJumpBurst();}}
  ...
  /* ── visual state ──
     A spring rather than a linear lerp: the body snaps back quickly then
     eases, which is what makes squash read as elastic. The rebound past 1
     on landing comes free from the spring overshooting. */
  dashStep();
  {
    const k=FX.squashSpring, d=FX.squashDamp;
    PV.sxv=(PV.sxv+(1-P.sx)*k)*d; P.sx+=PV.sxv;
    PV.syv=(PV.syv+(1-P.sy)*k)*d; P.sy+=PV.syv;
    /* Airborne shaping, driven by VERTICAL VELOCITY rather than a timer:
       stretch while rising fast, neutral near the apex, slight compress
       while falling. Applied as a bias on top of the spring so it never
       fights an active squash pose. */
    if(!P.grd&&PV.action!=="hurt"&&PV.action!=="land"){
      const v=clamp(P.vy/MAXV,-1,1);
      const air=1-Math.min(1,Math.abs(v)*1.15);   /* ~1 at the apex */
      const stretch=v<0?(-v)*.16:v*.10;
      P.sx+=((1-stretch*(v<0?1:-1)*.9)-P.sx)*.10*(1-air*.7);
      P.sy+=((1+stretch*(v<0?1:-1))-P.sy)*.10*(1-air*.7);
    }
    if(PV.actionT>0){PV.actionT--;if(PV.actionT<=0)PV.action="none";}
    if(PV.leanT>0){PV.leanT--;PV.lean*=.88;}else PV.lean*=.72;
    if(PV.dashGlow>0)PV.dashGlow=Math.max(0,PV.dashGlow-1/(FX.dashRingMs*.06));
    if(PV.landStrength>0)PV.landStrength=Math.max(0,PV.landStrength-.06);
  }
  /* A dash is a COMMITTED pose, not a tumble. Without this the airborne
     rotation keeps accumulating off the dash's large vx (~0.05 rad/frame),
     so by mid-dash the hero is at ~30-45 degrees and the afterimage trail
     reads as a chain of rotating diamonds rather than ghosts of a
     character. The lean supplies the tilt instead. */
  if(P.dash>0)P.rot*=.72;
  else P.rot=P.grd?P.rot*.7:clamp(P.rot+P.vx*.004+(P.air>34?P.face*.04:0),-.9,.9);
  if(iFr>0)iFr--;
  ...
  /* ── orbs ── */
  const mag=(has("magnet")?72:24)*(juice>0?4.2:1);
  for(const o of orbs){
    if(o.got)continue;
  ...
  if(juice>0)meter=0;
    ...
    pop(P.x+P.w/2,P.y-44,"JUICE MODE",juiceCol(),24);
    pop(P.x+P.w/2,P.y-22,"DOUBLE POINTS","#FFFFFF",12);
  ...
  for(const f of foes){
    ...
    switch(f.t){
      case"slime": case"spikey": case"cubebot": case"crystal":{
        const sp=(f.t==="spikey"?1:f.t==="cubebot"?.8:f.t==="crystal"?.34:1)*eSlow;
        f.x+=f.vx*sp;
  ...
  /* ── camera / world ── */
  /* Camera lead. During a dash the camera looks a little further ahead so
     the player can see what they are dashing into; it eases in and back out
     rather than snapping, and the extra amount is bounded so there is always
     usable space behind the player. camX itself is otherwise untouched, so
     boss positioning and procedural generation are unaffected. */
  const wantLead=P.dash>0?FX.dashLead*P.face:0;
  PV.camLead+=(wantLead-PV.camLead)*FX.dashLeadEase;
  if(Math.abs(PV.camLead)<.15)PV.camLead=0;
  const tgt=Math.max(0,P.x-W*.31+P.face*Math.min(90,W*.1)+PV.camLead);
  camX+=(tgt-camX)*.075;
  ...
  /* Ordinary running trail. EVERY field must be written, including dash:
     these are pooled slots, so a slot previously used by a dash afterimage
     keeps dash=1 and would go on rendering at dash strength — the running
     trail would inherit the dash look permanently after the first dash. */
  const tq=TRAIL[tIdx];tIdx=(tIdx+1)%TMAX;
  tq.x=P.x;tq.y=P.y;tq.rot=P.rot;tq.life=1;
  tq.sx=P.sx;tq.sy=P.sy;tq.face=P.face;tq.dash=0;tq.juice=juice>0;
  ...
  tutStep();
  /* Shake decay. shakeT is an impulse countdown: while it runs the impulse
     holds, then both axes decay together. Without it a dash held across
     several frames would keep re-arming the shake and never settle. */
  if(shakeT>0)shakeT--;
  else {shake*=.86;shakeY*=.86;if(shakeY<.02)shakeY=0;}
  flash*=.9;
}
...
function bossStep(){
  ...
  const BD=BOSSES[boss.k]||BOSSES.juice;
...
function draw(){
  /* Axis-weighted shake. shakeY only exceeds shake when an impulse asked
     for it; otherwise both axes read the same value and behave exactly as
     they did before. */
  const sY=Math.max(shakeY,0)||shake;
  const kx=shake?(Math.random()-.5)*shake:0,ky=sY?(Math.random()-.5)*sY:0;
  ...
  for(const o of orbs){
    ...
    if(o.fruit){
      ...
      ctx.save();ctx.translate(x,o.y+Math.sin(frame/16+o.ph)*2);
      ctx.fillStyle=F.col;ctx.beginPath();ctx.arc(0,0,7,0,7);ctx.fill();
      ctx.strokeStyle="#0B1226";ctx.lineWidth=1.6;ctx.stroke();
      ctx.strokeStyle="#4E8F3E";ctx.lineWidth=2;
      ctx.beginPath();ctx.moveTo(0,-6);ctx.lineTo(2,-11);ctx.stroke();
      ctx.fillStyle="#5FBF4F";ctx.beginPath();ctx.ellipse(5,-11,4,2,.5,0,7);ctx.fill();
      ctx.fillStyle="rgba(255,255,255,.55)";ctx.beginPath();ctx.arc(-2.4,-2.4,2,0,7);ctx.fill();
      ctx.restore();
    } else if(o.cry){
  ...
  if(!lite&&rsp&&ST==="play"){
    ...
    for(let i=0;i<TMAX;i++){
      ...
      /* Dash afterimages are stronger and reproduce the POSE they were
         captured in; the ordinary running trail stays very faint so it
         reads as motion blur rather than a queue of clones. */
      const a=t.dash?t.life*t.life*(t.juice?.62:.46):t.life*(juice>0?.4:.14);
      const sc=t.dash?(.82+t.life*.18):1;      /* older = smaller */
      ctx.save();ctx.globalAlpha=a;
      ctx.translate(t.x-camX+P.w/2,t.y+P.h/2);ctx.rotate(t.rot);
      ctx.scale((t.sx||1)*sc*(t.face<0?-1:1),(t.sy||1)*sc);
      ctx.drawImage(RSPR,-11-P.w/2,-15-P.h/2,44,60);
      ctx.restore();}
    ctx.globalAlpha=1;}

  /* ── particles ──
     Three kinds share one pool: 0 square (original), 1 speed streak,
     2 juice droplet. Streaks are stretched along their own velocity, which
     is what makes the dash read as speed rather than confetti. */
  for(let i=0;i<PMAX;i++){const q=PART[i];if(q.life<=0)continue;
    if(q.k===0){
      ctx.globalAlpha=q.life*.75;
      ctx.fillStyle=q.col||(q.gold?"#FFE9A0":"#A8B8D8");
      ctx.fillRect(q.x-camX-q.r/2,q.y-q.r/2,q.r,q.r);
    } else if(q.k===1){
      ctx.globalAlpha=q.life*.8;
      ctx.strokeStyle=q.col||"#A8B8D8";
      ctx.lineWidth=q.wid;ctx.lineCap="round";
      const L=q.len*q.life;
      ctx.beginPath();
      ctx.moveTo(q.x-camX,q.y);
      ctx.lineTo(q.x-camX-Math.cos(q.ang)*L,q.y-Math.sin(q.ang)*L);
      ctx.stroke();
    } else {
      ctx.globalAlpha=q.life*.85;
      ctx.fillStyle=q.col||"#FFE9A0";
      ctx.beginPath();ctx.arc(q.x-camX,q.y,q.wid*1.15,0,7);ctx.fill();
    }}
  ...
  if(ST==="play"){
    ...
    /* Translating to (P.y + P.h - ph/2) keeps the FEET planted while the
       body scales: squash grows the character downward-from-the-top rather
       than around its centre, so a landing never looks like the character
       teleported or sank into the platform. */
    ctx.translate(P.x-camX+(P.w-pw)/2+pw/2,P.y+(P.h-ph)+bob+ph/2);
    ctx.rotate(P.rot+PV.lean);
    ctx.translate(-pw/2,-ph/2);
    me().draw(ctx,pw,ph,P.face,kit,frame);
    ctx.restore();
    /* ── dash effect around the hero ──
       A compressed arc on the leading edge plus a thin highlight, both
       fading inside ~180ms. Deliberately small and off the face. */
    if(PV.dashGlow>0&&!lite){
      const g=PV.dashGlow, cx=P.x-camX+P.w/2, cy=P.y+P.h/2+bob;
      const col=juice>0?juiceCol():me().hex, d=PV.dashDir;
      ctx.save();
      ctx.globalAlpha=g*.5;
      ctx.strokeStyle=col;ctx.lineWidth=2+g*1.6;ctx.lineCap="round";
      /* leading-edge arc, opening away from the direction of travel */
      ctx.beginPath();
      ctx.arc(cx,cy,P.w*(.85+(1-g)*.75),
              (d>0?-.85:2.29),(d>0?.85:3.99));
      ctx.stroke();
      /* front highlight */
      ctx.globalAlpha=g*.8;ctx.lineWidth=2;
      ctx.beginPath();
      ctx.moveTo(cx+d*P.w*.52,cy-P.h*.26);
      ctx.lineTo(cx+d*P.w*.52,cy+P.h*.20);
      ctx.stroke();
      ctx.restore();ctx.lineCap="butt";ctx.globalAlpha=1;
    }
    if(juice>0){                            /* orbiting aura */
      const cx=P.x-camX+P.w/2,cy=P.y+P.h/2+bob;
      ctx.strokeStyle=juiceCol();ctx.lineWidth=2;
      ctx.globalAlpha=.55+.25*Math.sin(frame/7);
      ctx.beginPath();ctx.arc(cx,cy,P.w*1.15,0,7);ctx.stroke();
      ctx.globalAlpha=1;
      for(let i=0;i<3;i++){const a=frame/9+i*2.09;
        ctx.fillStyle=juiceCol();
        ctx.beginPath();ctx.arc(cx+Math.cos(a)*P.w*1.15,cy+Math.sin(a)*P.w*1.15,2.4,0,7);ctx.fill();}}
    ctx.globalAlpha=1;}
...
/* ══════ BOSS BODIES ══════════════════════════════════════════════
   One painter per boss so each reads as a different creature at a
   glance — silhouettes taken from the reference sheet. */
function bossBody(k,bx,by,body,trim,flap,stunned){
  if(!BOSSES[k])k="juice";
  ctx.fillStyle=body;
  if(k==="juice"){
    /* tentacle-mouthed blob: fat body, lolling tongue, one big eye */
    ctx.beginPath();ctx.ellipse(bx,by,34,26,0,0,7);ctx.fill();OUT(ctx,2.4);
    ctx.fillStyle=trim;
    for(let i=0;i<4;i++){const a=frame/14+i*1.57,tx=bx+Math.cos(a)*30,ty=by+18+Math.sin(a)*6;
      ctx.beginPath();ctx.ellipse(tx,ty,7,4.4,a,0,7);ctx.fill();}
    ctx.fillStyle="#3A1526";
    ctx.beginPath();ctx.ellipse(bx,by+9,17,9,0,0,7);ctx.fill();
    ctx.fillStyle=trim;
    ctx.beginPath();ctx.ellipse(bx+Math.sin(frame/11)*4,by+15,7,5,0,0,7);ctx.fill();
    ctx.fillStyle="#FFFFFF";
    for(let i=0;i<4;i++){const tx=bx-12+i*8;
      ctx.beginPath();ctx.moveTo(tx,by+4);ctx.lineTo(tx+4,by+11);ctx.lineTo(tx+8,by+4);ctx.closePath();ctx.fill();}
  } else if(k==="flame"){
    /* flame elemental: licking crown of fire */
    const f1=Math.sin(frame/3.6)*7,f2=Math.sin(frame/2.4+1)*5;
    ctx.beginPath();
    ctx.moveTo(bx,by-40-f1);
    ctx.quadraticCurveTo(bx+34,by-6,bx+22,by+22);
    ctx.quadraticCurveTo(bx,by+30,bx-22,by+22);
    ctx.quadraticCurveTo(bx-34,by-6,bx,by-40-f1);ctx.fill();OUT(ctx,2.2);
    ctx.fillStyle=trim;
    ctx.beginPath();
    ctx.moveTo(bx,by-20-f2);
    ctx.quadraticCurveTo(bx+16,by+2,bx,by+18);
    ctx.quadraticCurveTo(bx-16,by+2,bx,by-20-f2);ctx.fill();
  } else if(k==="rex"){
    /* horned brute: heavy jaw, spined back */
    ctx.beginPath();ctx.roundRect(bx-32,by-16,64,38,12);ctx.fill();OUT(ctx,2.4);
    ctx.fillStyle=body;
    for(let i=0;i<4;i++){ctx.beginPath();
      ctx.moveTo(bx-24+i*15,by-16);ctx.lineTo(bx-17+i*15,by-32);ctx.lineTo(bx-10+i*15,by-16);
      ctx.closePath();ctx.fill();}
    ctx.fillStyle="#2A0D14";
    ctx.beginPath();ctx.roundRect(bx-22,by+4,44,13,5);ctx.fill();
    ctx.fillStyle="#FFFFFF";
    for(let i=0;i<5;i++){const tx=bx-19+i*9;
      ctx.beginPath();ctx.moveTo(tx,by+4);ctx.lineTo(tx+4,by+12);ctx.lineTo(tx+8,by+4);ctx.closePath();ctx.fill();}
    ctx.fillStyle=trim;
    ...
    for(let i=0;i<8;i++){const a=i*.785+frame/60;
      ctx.beginPath();
      ctx.moveTo(bx+Math.cos(a)*28,by+Math.sin(a)*28);
      ctx.lineTo(bx+Math.cos(a)*(40+flap*.2),by+Math.sin(a)*(40+flap*.2));ctx.stroke();}
    ctx.lineCap="butt";
  }
}
...
function drawDebug(){
  ...
  ctx.fillStyle="rgba(11,18,38,.74)";
  ctx.fillRect(6,WY+6,bw,bh);
  ctx.strokeStyle="rgba(111,231,140,.5)";ctx.lineWidth=1;
  ctx.strokeRect(6,WY+6,bw,bh);
  ctx.fillStyle="#6FCF7F";
...
function drawHUD(){
  ...
  /* rows stack from a running cursor so an absent line never leaves a hole */
  let ry=T+70;
  ctx.font=F_MONO;
  ctx.fillStyle="rgba(255,201,74,.95)";ctx.fillText("◈ "+orbs_+(crys?"  ♦ "+crys:""),16,ry);ry+=16;
  ctx.fillStyle="rgba(168,184,216,.7)";ctx.fillText(wl.n,16,ry);ry+=16;
  ctx.font=F_SM;
  if(runFruit){ctx.fillStyle="rgba(155,239,255,.9)";ctx.fillText("FRUIT "+runFruit,16,ry);ry+=14;}
  if(BREW){ctx.fillStyle=BREWS[BREW].col;
    ctx.fillText(BREWS[BREW].icon+" "+BREWS[BREW].n.toUpperCase(),16,ry);ry+=14;}
  if(RMOD){ctx.fillStyle="rgba(255,201,74,.75)";
    ctx.fillText("DAILY · "+modText().toUpperCase(),16,ry);ry+=14;}
  ...
  if(juice>0){
    ...
    /* final 3s pulse, and a faster flash in the last second, so the end of
       Juice Mode is read ahead of time instead of noticed after the fact */
    const warn=juice<=180, last=juice<=60;
    ctx.fillStyle=last?(frame%4<2?"#FFFFFF":"#FF5F7A")
                 :warn?(frame%10<5?"#FFFFFF":juiceCol())
                 :juiceCol();
    ctx.fillRect(bx,T+14,bw*k,8);
    /* the 15s ceiling drawn as a tick, so the cap is visible not just coded */
    if(juiceMax<JUICE_CAP_F){
      const cx=bx+bw*(juiceMax/JUICE_CAP_F);
      ctx.fillStyle="rgba(255,255,255,.28)";ctx.fillRect(cx-1,T+12,1,12);
    }
    ctx.font=F_TAG;ctx.fillStyle=last?"#FF9BAA":"#FFFFFF";
    ctx.fillText("JUICE MODE  "+(juice/60).toFixed(1)+"s"+
      (juiceExt>0?"  (+"+(juiceExt/60).toFixed(1)+"s)":""),W/2,T+36);
    ctx.font=F_SM;ctx.fillStyle="rgba(255,255,255,.75)";
    ctx.fillText(juice>=JUICE_CAP_F?"MAX JUICE · ♦ CANNOT EXTEND FURTHER"
                :"×2 DAMAGE · INVINCIBLE · ♦ CRYSTAL = +0.5s",W/2,T+48);
  } else {
  ...
  ctx.font=F_SM;ctx.textAlign="right";
  ctx.fillStyle=P.dashCd>0?"rgba(168,184,216,.6)":"rgba(111,207,127,.9)";
  ctx.fillText(P.dashCd>0?"DASH":(dashChg>0?"CHARGING":"DASH READY"),W-18,Bm-28);
  ...
  if(dashChg>0&&P.dashCd<=0){
    ctx.save();
    ctx.translate(W-44,Bm-52);
    ctx.strokeStyle="rgba(168,184,216,.35)";ctx.lineWidth=3;
    ctx.beginPath();ctx.arc(0,0,13,0,7);ctx.stroke();
    ctx.strokeStyle="#9BEFFF";ctx.lineWidth=3;ctx.lineCap="round";
    ctx.beginPath();ctx.arc(0,0,13,-Math.PI/2,-Math.PI/2+dashChg*Math.PI*2);ctx.stroke();
    ctx.lineCap="butt";
    ctx.restore();
  }
  ...
  if(ABIL[activePass]){
    ctx.fillStyle="rgba(168,184,216,.7)";
    ctx.fillText(me().id.toUpperCase()+" · "+ABIL[activePass].short,W-18,Bm-42);}
  ctx.textAlign="left";
  ...
  if(boss){
    ...
    ctx.textAlign="center";
    if(boss.st==="warn"){
      ctx.fillStyle="rgba(255,95,122,"+(.14+.16*Math.sin(frame/4))+")";ctx.fillRect(0,WY,W,WH);
      ctx.fillStyle="#FFE9A0";ctx.font="700 clamp(18px,4.4vw,30px) 'Space Grotesk',system-ui,sans-serif";
      ctx.fillText("⚠  "+(BOSSES[boss.k]||BOSSES.juice).name+" INCOMING",W/2,BC);
      ctx.font=F_MONO;ctx.fillStyle="rgba(255,255,255,.9)";
      ctx.fillText((BOSSES[boss.k]||BOSSES.juice).tag,W/2,BC+26);
      if(SAVE.bossKills<1){ctx.fillStyle="#6FCF7F";
        ctx.fillText("DODGE THE DIVE — THEN STOMP THE GLOWING CORE",W/2,BC+46);}
    } else if(boss.st==="dead"){
      if(boss.t>60){
        ctx.fillStyle="#FFE9A0";ctx.font="700 clamp(17px,4vw,26px) 'Space Grotesk',system-ui,sans-serif";
        ctx.fillText((BOSSES[boss.k]||BOSSES.juice).name+" DEFEATED",W/2,BC);
        ctx.font=F_MONO;ctx.fillStyle="rgba(255,255,255,.9)";
        ctx.fillText("+"+(boss.reward||0)+" ◈",W/2,BC+24);
        if(boss.gift){ctx.fillStyle="#6FCF7F";
          ctx.fillText("HERO UNLOCKED · "+boss.gift.toUpperCase(),W/2,BC+42);}
        if(boss.drop){ctx.fillStyle="#9BEFFF";
          ctx.fillText("+"+boss.drop.n+" "+FRUIT[boss.drop.k].icon+" "+FRUIT[boss.drop.k].n.toUpperCase(),W/2,BC+60);}}
    } else if(boss.st!=="retreat"){
      const BH=BOSSES[boss.k]||BOSSES.juice;
      /* health as hearts, exactly as sketched, with a bar behind for
         partial damage; both at the top of the screen */
      const n=boss.max,per=Math.min(22,Math.max(11,(W*.5)/n));
      const tw=n*per, tx=W/2-tw/2, hy=T+72;
      ctx.font=F_SM;ctx.fillStyle="rgba(255,255,255,.9)";
      ctx.fillText(BH.name+"   PHASE "+boss.phase+"/3",W/2,hy-8);
      for(let i=0;i<n;i++){
        const full=i<boss.hp, hx0=tx+i*per, cy=hy+7;
        ctx.fillStyle=full?(boss.phase===3?"#FF3D5F":BH.col):"rgba(255,255,255,.14)";
        if(full&&boss.hurt>0&&frame%6<3)ctx.fillStyle="#FFFFFF";
        ctx.beginPath();
        ctx.moveTo(hx0+per*.42,cy+per*.30);
        ctx.bezierCurveTo(hx0-per*.06,cy+per*.02,hx0+per*.08,cy-per*.16,hx0+per*.42,cy+per*.06);
        ctx.bezierCurveTo(hx0+per*.76,cy-per*.16,hx0+per*.90,cy+per*.02,hx0+per*.42,cy+per*.30);
        ctx.fill();}
      if(boss.phase===3){ctx.font=F_TAG;ctx.fillStyle="rgba(255,61,95,"+(.6+.4*Math.sin(frame/5))+")";
        ctx.fillText("RAGE",W/2,hy+34);}
      if(boss.st==="stun"&&SAVE.bossKills<1){
        ctx.font=F_MONO;ctx.fillStyle="#6FCF7F";
        ctx.fillText("NOW! JUMP ON IT",W/2,WY+WH*.24);}
    }
    ctx.textAlign="left";}
  ...
  if(evt&&frame-evt.t0<150){
    ...
    ctx.fillText(({crystal:"CRYSTAL RAIN",meteor:"METEOR SHOWER",ufo:"UFO SIGHTING",
      tornado:"TORNADO",dragon:"DRAGON PASSING",rainbow:"RAINBOW BRIDGE"})[evt.k],W/2,WY+64);
    ctx.textAlign="left";}
...
function frameLoop(now){
  ...
  acc+=dt*timeScale;
  /* Phase timing is opt-in (profN>0, set only by the debug profile hook).
     A shipped build never calls performance.now() here — measuring every
     frame to answer a question nobody asked is its own perf bug. */
  const prof=profN>0;
  let t0=prof?performance.now():0;
  let n=0;
  while(acc>=1/60&&n++<5){step();acc-=1/60;}
  if(acc>1/6)acc=0;
  if(prof){profSim+=performance.now()-t0;t0=performance.now();}

  musicTick();
  syncPadState();
  if(saver()&&(++skip&1)){if(prof){profSkip++;profN--;}return;}
  draw();
  if(prof){profDraw+=performance.now()-t0;profN--;}
}
requestAnimationFrame(frameLoop);
...
function paintLab(){
  ...
  Object.keys(BREWS).forEach(k=>{
    ...
    el.innerHTML='<span></span><span class="v"></span><span class="track" style="height:auto;background:none">'+
                 '<span style="font-family:var(--mono);font-size:8.5px;color:var(--haze)"></span></span>';
  ...
  SKINS.forEach(k=>{
    ...
    if(on){l.style.color="var(--amber)";l.style.borderColor="rgba(255,201,74,.45)";
           l.style.background="rgba(255,201,74,.07)";}
...
function guideText(){
  ...
  const b=!t?"<div><b>A / D</b> or <b>← →</b> to steer · <b>SHIFT</b> to dash · <b>P</b> to pause</div>"
          :OPT.scheme==="buttons"
           ?"<div><b>◀ ▶</b> to steer · <b>▲</b> to jump · <b>»</b> to dash</div>"
           :"<div><b>SLIDE</b> your finger to steer · <b>HOLD STILL</b> to charge a dash</div>";
  ...
  const e="<div>You have <b>3 HEARTS</b> of health per run · grab a <b>♦ crystal</b> in Juice Mode for <b>+0.5s</b></div>";
...
/* iOS gets the button on sight, since the event that would reveal it on other
   browsers is never coming. Gated on being served, though: nothing can be
   installed from file://, so offering it there would be a button that cannot
   work. Not shown once already installed. */
if(IOS&&!STANDALONE()&&location.protocol.startsWith("http"))showInstallBtn(true);
...
if(/[?&]debug=1\b/.test(location.search)){
  window.JJA_DEBUG={
    get state(){return{
      ...
      elites,bossWins,RMOD,activePass,autoLite,timeScale,dpr:DPR,
    ...
    density(startM,sweeps){
      ...
      for(const o of orbs){if(o.fruit)fru++;else if(o.cry)cry++;else orb++;}
    ...
    /* Clears the adaptive downgrade so a measurement can start from full
       quality. Without this, once auto-lite has engaged it is already
    ...
    /* ── game-feel surface ──
       Exposes the visual state and the effect pools so the feel pass can be
       tested for real rather than eyeballed: counts of live particles by
       kind, live afterimages, and the pose priority machine. */
    feel(){
      let sq=0,st=0,dr=0,tr=0,trDash=0;
      for(let i=0;i<PMAX;i++){const q=PART[i];if(q.life<=0)continue;
        if(q.k===1)st++;else if(q.k===2)dr++;else sq++;}
      for(let i=0;i<TMAX;i++){const t=TRAIL[i];if(t.life>0){tr++;if(t.dash)trDash++;}}
      return{action:PV.action,actionT:PV.actionT,sx:P.sx,sy:P.sy,
             lean:PV.lean,dashGlow:PV.dashGlow,landStrength:PV.landStrength,
             camLead:PV.camLead,shake,shakeY,shakeT,hstop,
             parts:{square:sq,streak:st,droplet:dr,total:sq+st+dr},
             trail:tr,trailDash:trDash,
             dash:P.dash,dashCd:P.dashCd,jumpsUsed:P.jumpsUsed,grd:P.grd,
             calm:calm(),saver:saver()};},
    setPose(a,f,sx,sy){return setPose(a,f,sx,sy);},
    clearPose(){clearPose();return true;},
    dash(){tryDash();return P.dash>0;},
    jump(){tryJump();return true;},
    /* land at a chosen impact fraction, to exercise each tier */
    testLand(imp){landingFeedback(clamp(imp,0,1),P.y+P.h);return PV.action;},
    fx:()=>FX,
    schemeSaved:()=>schemeWasSaved,
    ...
    /* Samples the real loop for n frames and returns the per-phase cost in
       milliseconds. Returns a promise so a caller gets the finished sample
       rather than a snapshot of a measurement still in progress. */
    profile(n){
      profSim=profDraw=profSkip=0;profN=Math.max(1,n|0);
      const want=profN;
      return new Promise(res=>{
        const wait=()=>{
          if(profN>0)return requestAnimationFrame(wait);
          const drawn=want-profSkip;
          res({sim:profSim/want,draw:profDraw/Math.max(1,drawn),
               skipped:profSkip,n:want});
        };
