...
function load(){
  const go=()=>{rollStreak();rollMissions();rollMod();paintAll();bootDone();};
  let got=null;
...
/*32*/{id:"Inferno",hex:"#FF5A2B",eye:"#FFE9A0",rar:4,pas:"power",draw(c,w,h,f,k,t){
...
function juiceLen(){
  let n=has("juicelong")?720:480;
  if(has("speed"))n*=.6;                  /* Bolt: fast but burns juice */
  if(BREW==="rainbow")n*=1.6;
  if(RMOD==="JUICERUSH")n*=.6;
  return Math.round(n);}
...
function damageFoe(f){
  const dmg=((has("slayer")||has("power"))?2:1)*(juice>0?2:1);      /* Juice Mode now boosts damage */
  f.hp-=dmg;f.flashT=8;
  sfx("stomp");buzz(12);combo++;comboT=frame;gainJuice(f.el?18:6);
...
function step(){
  ...
  const spd=SPD*(RMOD==="FAST"?1.34:1)*(has("speed")?1.25:1)
  ...
  if(P.dash<=0){
    const gm=(wl.mod==="lowg"?.72:1)*(has("power")?.86:1);  /* Inferno floats: harder */
  ...
  for(const o of orbs){
    ...
    if(dd<mag*mag){
      o.got=true;
      const mult=(1+Math.floor(combo/8)+(juice>0?2:0))*(juice>0&&BREW==="rainbow"?3:1);
      if(o.fruit){
      ...
      } else if(o.cry){crys++;orbs_+=10*mult;sfx("cry");buzz(20);if(juice<=0)gainJuice(26);flash=.5;
...
function bossDamage(mult){
  const dmg=((has("slayer")||has("power"))?2:1)*(juice>0?2:1)*(mult||1);
...
function bossStep(){
  ...
  switch(boss.st){
    ...
    case"stun":{
      ...
      if(boss.t>(p3?95:p2?110:155)){boss.st="hover";boss.t=0;boss.cool=60;}
      break;}
  }
...
function drawBoss(lite){
  ...
  /* the weak point — the whole point of the redesign is that the player
     can see where and when to hit */  /* the weak point — the whole point of the redesign is that the player
...
function drawHUD(){
  ...
  if(juice>0){
    ...
    ctx.fillStyle=juice<90&&frame%8<4?"#FFFFFF":juiceCol();
  ...
  ctx.fillText(P.dashCd>0?"DASH":"DASH READY",W-18,Bm-28);
