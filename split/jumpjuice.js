...
function adopt(raw){
  ...
  /* The roster was renamed in the visual redesign, so a save written by an
     earlier build lists hero ids that no longer exist. Those entries are
     dropped by the filter above (they cannot be drawn), but the six launch
     heroes are always granted, so a returning player is never left with a
     smaller roster than a brand-new one. Coins, XP, awards, fruit and the
     best distance are all untouched. */
  SAVE.unlocked=[...new Set(STARTERS.concat(un))];
